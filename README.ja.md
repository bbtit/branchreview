...
従来の差分表示はノイズが多くなりがちです。変更前と変更後が左右に並び、消えた行も画面に残り、変更後のコードを対応関係ごと追うことになります。エディタの機能もすべて使えるとはかぎりません。定義へ飛ぶ、参照を探す、リネームする。差分の画面では、そうした操作ができないことがあります。

BranchReview は、レビュー対象のブランチを普通のエディタで開いたまま、ブランチ間の差分を gutter のマークと hover で表示する VS Code 拡張です。読んでいるのは実際のプロジェクトにある実際のファイルなので、変更後のコードに集中でき、エディタ本来の機能もすべて使えます。
